...
static int readline(char *line, int length, FILE *stream)
{
	...
	while (size > 0)
	{
		...
		/* If ESC, lets check if belongs to a know sequence. */
		else if (ch == ESC)
		{
			char *oldline;
			...
			/* Read our 'target' character and skip if different. */
			if (read(fd, &ch, 1) != 1 || (ch != KUP && ch != KDOWN))
				continue;
